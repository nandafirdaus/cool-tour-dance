...
namespace GameNuclex.Screen
{
    class SelectDance : Scene
    {
        ...
        public SelectDance(Engine engine) : base(engine) 
        {
        ...
        protected override void OnEntered()
        {
            ...
            for (int i = 0; i < allList.Length; i++)
            {
                Pictures[i] = Texture2D.FromStream(engine.graphics,
                    File.OpenRead(GameIO.DIR_PATH + allList[i] + "\\picture.jpg"));
            }
        ...
        void CheckCollision(GameTime gameTime)
        {
            ...
            if (rectangle1.Intersects(rectangle2))
            {
                ...
                if (progressBar.isFull)
                {
                    Play play = new Play(engine, allList[currentIndex]);
